...
log_file = os.path.join(script_dir, 'flow_timeout.log')
...
def flow_timeout():
    ...
    # Add a flow with a timeout to the switch
    info('*** Adding flow with timeout\n')
    logging.info('Adding flow with timeout')
    s1.cmd('ovs-ofctl add-flow s1 "priority=1,actions=normal,idle_timeout=10"')
