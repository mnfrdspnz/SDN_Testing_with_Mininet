...
log_file = os.path.join(script_dir, 'load_balancing.log')
...
def load_balancing():
    ...
    # Configure load balancing on the switches
    info('*** Configuring load balancing\n')
    logging.info('Configuring load balancing')
    s1.cmd('ovs-ofctl add-flow s1 "in_port=1,actions=2,3"')
    s2.cmd('ovs-ofctl add-flow s2 "in_port=1,actions=2,3"')
    s3.cmd('ovs-ofctl add-flow s3 "in_port=1,actions=2,3"')
