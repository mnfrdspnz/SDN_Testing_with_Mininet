...
log_file = os.path.join(script_dir, 'visualization.log')
...
def visualization():
    ...
    info('*** Configuring data visualization\n')
    logging.info('Configuring data visualization')
    s1.cmd('ovs-vsctl -- --id=@m create mirror name=m0 select-all=true output-port=s1-eth1 -- set bridge s1 mirrors=@m')

    # Collect and visualize data
    info('*** Collecting and visualizing data\n')
    logging.info('Collecting and visualizing data')
    visualization_data = s1.cmd('ovs-vsctl list mirror m0')
    logging.info(f'Visualization data: {visualization_data}')
