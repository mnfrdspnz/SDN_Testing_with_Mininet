...
log_file = os.path.join(script_dir, 'traffic_analysis.log')
...
def traffic_analysis():
    ...
    info('*** Configuring traffic analysis\n')
    logging.info('Configuring traffic analysis')
    s1.cmd('ovs-vsctl -- --id=@m create mirror name=m0 select-all=true output-port=s1-eth1 -- set bridge s1 mirrors=@m')

    # Collect traffic analysis data
    info('*** Collecting traffic analysis data\n')
    logging.info('Collecting traffic analysis data')
    traffic_data = s1.cmd('ovs-vsctl list mirror m0')
    logging.info(f'Traffic analysis data: {traffic_data}')
