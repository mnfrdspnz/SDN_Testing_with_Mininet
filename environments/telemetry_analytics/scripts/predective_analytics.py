...
log_file = os.path.join(script_dir, 'predictive_analytics.log')
...
def predictive_analytics():
    ...
    info('*** Configuring predictive analytics\n')
    logging.info('Configuring predictive analytics')
    s1.cmd('ovs-vsctl -- --id=@m create mirror name=m0 select-all=true output-port=s1-eth1 -- set bridge s1 mirrors=@m')

    # Collect and analyze data for predictions
    info('*** Collecting and analyzing data for predictions\n')
    logging.info('Collecting and analyzing data for predictions')
    predictive_data = s1.cmd('ovs-vsctl list mirror m0')
    logging.info(f'Predictive analytics data: {predictive_data}')
