...
def policy_enforcement():
    ...
    # Configure service function chain with policy enforcement
    info('*** Configuring service function chain with policy enforcement\n')
    logging.info('Configuring service function chain with policy enforcement')
    ...
    # Add policy enforcement to the chain
    info('*** Adding policy enforcement to the chain\n')
    logging.info('Adding policy enforcement to the chain')
    s2.cmd('ovs-ofctl add-flow s2 "in_port=chain1,ip,nw_dst=10.0.0.2,actions=drop"')

    # Test connectivity with policy enforcement
    info('*** Testing connectivity with policy enforcement\n')
    logging.info('Testing connectivity with policy enforcement')
    result = net.ping([h1, h2])
    logging.info(f'Ping result with policy enforcement: {result}')

    # Remove policy enforcement from the chain
    info('*** Removing policy enforcement from the chain\n')
    logging.info('Removing policy enforcement from the chain')
    s2.cmd('ovs-ofctl del-flows s2 "in_port=chain1,ip,nw_dst=10.0.0.2,actions=drop"')

    # Test connectivity after removing policy enforcement
    info('*** Testing connectivity after removing policy enforcement\n')
    logging.info('Testing connectivity after removing policy enforcement')
    result = net.ping([h1, h2])
    logging.info(f'Ping result after removing policy enforcement: {result}')
