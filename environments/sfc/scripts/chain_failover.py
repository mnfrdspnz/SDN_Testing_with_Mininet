...
log_file = os.path.join(script_dir, 'chain_failover.log')
...
def chain_failover():
    ...
    # Configure service function chain with failover
    info('*** Configuring service function chain with failover\n')
    logging.info('Configuring service function chain with failover')
    ...
    info('*** Configuring failover paths\n')
    logging.info('Configuring failover paths')
    s2.cmd('ovs-ofctl add-flow s2 "in_port=chain1,actions=output:chain6"')
    s3.cmd('ovs-ofctl add-port s3 chain6 -- set interface chain6 type=patch options:peer=chain7')
    s3.cmd('ovs-ofctl add-flow s3 "in_port=chain7,actions=output:chain3"')

    # Test connectivity with failover paths
    info('*** Testing connectivity with failover paths\n')
    logging.info('Testing connectivity with failover paths')
    result = net.ping([h1, h2])
    logging.info(f'Ping result with failover paths: {result}')

    # Simulate failover
    info('*** Simulating failover\n')
    logging.info('Simulating failover')
    s2.cmd('ovs-ofctl del-flows s2 "in_port=chain1,actions=output:chain2"')
