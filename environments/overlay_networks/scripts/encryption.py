...
log_file = os.path.join(script_dir, 'encryption.log')
...
def encryption():
    ...
    # Configure encrypted VXLAN tunnel
    info('*** Configuring encrypted VXLAN tunnel\n')
    logging.info('Configuring encrypted VXLAN tunnel')
    s1.cmd('ovs-vsctl add-port s1 vxlan0 -- set interface vxlan0 type=vxlan options:remote_ip=10.0.0.2 options:key=flow options:df_default=true')
    s2.cmd('ovs-vsctl add-port s2 vxlan0 -- set interface vxlan0 type=vxlan options:remote_ip=10.0.0.1 options:key=flow options:df_default=true')
    
    # Set VXLAN flows with encryption
    ...
    info('*** Testing encrypted traffic\n')
    logging.info('Testing encrypted traffic')
    h1.cmd('iperf -s &')
    h2.cmd('iperf -c 10.0.0.1')
    logging.info('Encrypted traffic test completed')
