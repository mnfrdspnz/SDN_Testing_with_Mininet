...
log_file = os.path.join(script_dir, 'dynamic_tunnels.log')
...
def dynamic_tunnels():
    ...
    # Dynamically create VXLAN tunnel
    info('*** Dynamically creating VXLAN tunnel\n')
    logging.info('Dynamically creating VXLAN tunnel')
    ...
    # Dynamically delete VXLAN tunnel
    info('*** Dynamically deleting VXLAN tunnel\n')
    logging.info('Dynamically deleting VXLAN tunnel')
    s1.cmd('ovs-vsctl del-port s1 vxlan0')
    s2.cmd('ovs-vsctl del-port s2 vxlan0')

    # Test connectivity after tunnel deletion
    info('*** Testing connectivity after tunnel deletion\n')
    logging.info('Testing connectivity after tunnel deletion')
    result = net.ping([h1, h2])
    logging.info(f'Ping result after tunnel deletion: {result}')
