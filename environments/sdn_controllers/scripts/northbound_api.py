...
log_file = os.path.join(script_dir, 'northbound_api.log')
...
def northbound_api():
    ...
    # Example interaction with the northbound API
    info('*** Interacting with Northbound API\n')
    logging.info('Interacting with Northbound API')
    url = "http://127.0.0.1:8080/api/v1/northbound/"
    response = requests.get(url)
    logging.info(f'Northbound API response: {response.json()}')
