...
log_file = os.path.join(script_dir, 'controller_failover.log')
logging.basicConfig(filename=log_file, level=logging.INFO)

def controller_failover():
    # Create a Mininet object with two remote controllers
    net = Mininet(controller=RemoteController)

    # Add two remote controllers with IP 127.0.0.1 and different ports
    ...
    # Simulate controller failover by stopping one controller
    info('*** Simulating controller failover\n')
    logging.info('Simulating controller failover')
    net.get('c0').stop()
