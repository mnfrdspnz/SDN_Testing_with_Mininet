...
log_file = os.path.join(script_dir, 'security_policy.log')
...
def security_policy():
    ...
    # Enforce security policies using the SDN controller
    info('*** Enforcing security policies\n')
    logging.info('Enforcing security policies')
    # Example: Add security policy rules
    s1.cmd('ovs-ofctl add-flow s1 "priority=10,ip,nw_src=10.0.0.1,nw_dst=10.0.0.2,actions=drop"')
    logging.info('Added security policy to drop traffic from 10.0.0.1 to 10.0.0.2')
