...
log_file = os.path.join(script_dir, 'southbound_api.log')
...
def southbound_api():
    ...
    # Example interaction with the southbound API
    info('*** Interacting with Southbound API\n')
    logging.info('Interacting with Southbound API')
    # Example: Adding a flow rule to the switch
    s1.cmd('ovs-ofctl add-flow s1 "priority=1,actions=normal"')
    logging.info('Added flow rule to switch s1')
